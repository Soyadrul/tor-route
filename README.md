...
Route all of your machine's internet traffic — terminal apps (`curl`, `git`, `ssh`), browsers, daemons and everything else — through Tor with a single command, while automatically preventing DNS, IPv6 and UDP leaks.

## Why this exists

Tor Browser only protects traffic generated by the browser itself. Everything else — terminal commands (`curl`, `git`, SSH, package managers, API clients) and even the regular browser — still uses your normal internet connection unless you manually configure Tor or use a VPN. `tor-route` provides a one-command way to temporarily route **all** of your Linux machine's TCP traffic through Tor (any application, not just the terminal), while preventing common DNS, IPv6 and UDP leaks, then restore the original networking configuration when you're done.

> Works on any Linux distro using one of the following init systems: Systemd, OpenRC, Runit or SysVinit. Tested on [Arch Linux](https://archlinux.org/).
